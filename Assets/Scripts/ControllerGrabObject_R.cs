...
public class ControllerGrabObject_R : MonoBehaviour
{
    ...
    private void GrabObject() //물체 잡는 것
    {
        ...
        if (objectInHand.GetComponent<PhotonView>().ownerId != PhotonNetwork.player.ID)
        {
    ...
    private void ReleaseObject()// 물체 놓는 것 
    {
        if (GetComponent<FixedJoint>()) //컨트롤러 오브젝트에 포함된 FixedJoint 컴포넌트가 있는지 확인 
        {
            ...
            objectInHand.GetComponent<Rigidbody>().velocity = SteamVR_Input._default.inActions.Pose.GetVelocity(SteamVR_Input_Sources.RightHand)*2;
            objectInHand.GetComponent<Rigidbody>().angularVelocity = SteamVR_Input._default.inActions.Pose.GetAngularVelocity(SteamVR_Input_Sources.RightHand);
        }
