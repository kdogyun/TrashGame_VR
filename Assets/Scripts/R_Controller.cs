...
/*    void Start()
            ...
            if (Vector3.Distance(col.transform.position, R_Hand.transform.position) != 0)
            {
                if (mo != null)
                {
                    if (SteamVR_Input._default.inActions.GrabPinch.GetState(SteamVR_Input_Sources.RightHand))
                    {
                        ...
                        var ballPhotonView = col.GetComponent<PhotonView>();
                        ballPhotonView.TransferOwnership(playerPhotonView.ownerId);
    ...
    {
        if (!SteamVR_Input._default.inActions.GrabPinch.GetState(SteamVR_Input_Sources.RightHand) && mo != null)
        {
            if (mo != null)
            {
                mo.Isholded = false;
